...
{
    VkResult result = vkAcquireNextImageKHR(context->device.logical_device , swapchain->handel, timeout_ns, image_available_semaphore, fence, out_image_index);
...
)
{
    ...
    present_info.pSwapchains = &swapchain->handel;
...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain)
{
    ...
    //Setup the Queue Family Index
    if(context->device.graphics_queue_index != context->device.present_queue_index)
    {
        u32 queueFamilyIndieces[] = 
        {
        ...
        swapchain_create_info.pQueueFamilyIndices = queueFamilyIndieces;
    }
    else
    {
        swapchain_create_info.preTransform = 0; // VK_SHARING_MODE_EXCLUSIVE
    ...
    swapchain_create_info.oldSwapchain = 0; // TODO: Change it later 

    VK_CHECK(vkCreateSwapchainKHR(context->device.logical_device, & swapchain_create_info, context->allocator, &swapchain->handel));

    //Start with zero frame index
    context->current_frame = 0;
    //Images
    swapchain->image_count = 0;
    VK_CHECK(vkGetSwapchainImagesKHR(context->device.logical_device, swapchain->handel, &swapchain->image_count, 0));
    if(!swapchain->images)
    {
        swapchain->images = (VkImage*)fallocate(sizeof(VkImage) * swapchain->image_count, MEMORY_TAG_REDERING);
    }
    if(!swapchain->views)
    {
        swapchain->views = (VkImageView*)fallocate(sizeof(VkImageView) * swapchain->image_count, MEMORY_TAG_REDERING);
    }
    VK_CHECK(vkGetSwapchainImagesKHR(context->device.logical_device, swapchain->handel, &swapchain->image_count, swapchain->images));
...
void destroy(vulkan_context* context, vulkan_swapchain* swapchain)
{
    ...
    vkDestroySwapchainKHR(context->device.logical_device, swapchain->handel, context->allocator);
}