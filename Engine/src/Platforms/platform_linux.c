...
b8 Platform_pump_messages(platform_state* state)
{
    ...
    while(event != 0)
    {
        ...
        switch(event->response_type & -0x80)
        {
            ...
            {
                //KeyPress Event
                xcb_key_press_evemt_t *kb_event = (xcb_key_press_event_t *)event;
                b8 pressed = event->response_type == XCB_KEY_PRESS;
                xcb_leycode_t code = kb_event->detail;
            ...
            case XCN_BUTTON_RELEASED:
            {
                ...
                button mouse_button = BUTTON_MAX_BUTTONS;
